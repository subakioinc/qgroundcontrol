...
void Vehicle::_commonInit(void)
{
    ...
    _addFactGroup(&_gasFactGroup,               _gasFactGroupName);   //jaeeun
...
void Vehicle::_handleGas(mavlink_message_t& message)
{
    ...
    _gasFactGroup._pf1Fact.setRawValue(gas.integrated_x); //gas.intertaed_x : CDC1 + gas.intergrated_y : CDC2 계산 필요 
    ...
    , _humFact      (0, _humFactName,       FactMetaData::valueTypeDouble)
{
    ...
    _pf1Fact.setRawValue      (std::numeric_limits<float>::quiet_NaN());
    _pf2Fact.setRawValue      (std::numeric_limits<float>::quiet_NaN());
    _tempFact.setRawValue      (std::numeric_limits<float>::quiet_NaN());
    _humFact.setRawValue      (std::numeric_limits<float>::quiet_NaN());
}
